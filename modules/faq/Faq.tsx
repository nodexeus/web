...
const faqs = [
  {
    title: 'What is BlockVisor’s Mission?',
    text: 'Our mission is simply to help the proliferation of Web3. For this, we need infrastructure that is designed for Web3 and accessible for anyone to operate in a decentralized manner. If Amazon AWS is considered the infrastructure for Web 2.0, then we would like BlockVisor to be considered the infrastructure for Web3.',
  },
  {
    title: 'Why BlockVisor?',
    text: 'BlockVisor was originally the largest validator-as-a-service operator on the Helium network and operated over 50% of the Helium network’s validators at one point or another. When we started, we estimated that running equivalent performing validators on Amazon AWS was costing around $200/mo/validator compared to our operating costs of only $11/mo/validator. However, running a validators-as-a-service model didn’t feel right to us. Blockchains are meant to be resilient and the majority of this resilience comes from being decentralized. But many of our customers came to us because it was such a challenge to deploy their validators, to constantly keep them updated and to keep them performing well. We really didn’t think running your own validator should be that hard–in fact we wanted to encourage more people to run their own validators since that makes the overall network far more valuable. So we set about taking our platform and turning it into a tool that would allow anyone to easily deploy and manage any blockchain node anywhere in the world with a simple click of a button. And so here we are. Still a work in progress but today, using BlockVisor should be drop dead simple for companies and individuals to efficiently deploy and manage their own nodes. We hope this allows people to grow their projects and businesses and use BlockVisor as an infrastructure tool in place of traditional cloud providers.',
  },
  {
    title: 'How Does BlockVisor Work?',
    text: 'BlockVisor acts as an orchestration layer allowing customer nodes to run on bare metal servers in any data center or location the customer chooses. Customers can optionally run fully managed blockchain nodes and servers within BlockVisor’s data centers for convenience, however our services are designed to allow customers to run on any server and remain in full control of their own compute resources. We also make use of several patent pending technologies to make running nodes more efficient and to support on boarding new chains in days instead of months. <br /><br /> Stay tuned as we will be releasing a series of “deep dives” into how the underlying technology works.',
  },
  {
