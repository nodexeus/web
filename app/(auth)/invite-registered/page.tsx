'use client';

import { useEffect, Suspense } from 'react';
import { useRouter, useSearchParams } from 'next/navigation';
import { invitationClient } from '@modules/grpc/clients/invitationClient';
import { userClient } from '@modules/grpc/clients/userClient';
import { toast } from 'sonner';
import { Loader2 } from 'lucide-react';

export default function InviteRegisteredPage() {
  return (
    <Suspense>
      <InviteRegisteredContent />
    </Suspense>
  );
}

function InviteRegisteredContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const invitationId = searchParams.get('invitation_id') || '';

  useEffect(() => {
    if (!invitationId) {
      router.replace('/nodes');
      return;
    }

    (async () => {
      try {
        // Read current user identity
        const raw = localStorage.getItem('identity');
        if (!raw) {
          // Not logged in — redirect to login with invitation context
          const url = new URL('/login', window.location.origin);
          url.searchParams.set('invitation_id', invitationId);
          url.searchParams.set('invited', 'true');
          router.replace(url.pathname + url.search);
          return;
        }

        const identity = JSON.parse(raw);
        const email = identity.email;

        if (!email) {
          const url = new URL('/login', window.location.origin);
          url.searchParams.set('invitation_id', invitationId);
          url.searchParams.set('invited', 'true');
          router.replace(url.pathname + url.search);
          return;
        }

        // Fetch received invitations to find the matching one
        const receivedInvitations =
          await invitationClient.receivedInvitations(email);
        const activeInvitation = receivedInvitations.find(
          (i: any) => i.invitationId === invitationId,
        );

        // Accept the invitation
        await invitationClient.acceptInvitation(invitationId);

        // Set the invited org as default
        if (activeInvitation && identity.userId) {
          try {
            await userClient.updateSettings(
              identity.userId,
              'defaultOrg',
              JSON.stringify({
                orgId: activeInvitation.orgId,
                name: activeInvitation.orgName,
              }),
            );
          } catch {
            // Non-critical — org will still be accessible
          }
        }

        toast.success(`Joined ${activeInvitation?.orgName || 'organization'}`);
        router.replace('/nodes');
      } catch (err) {
        console.error('Failed to accept invitation:', err);
        const errorUrl = new URL('/login', window.location.origin);
        errorUrl.searchParams.set('invitation_id', invitationId);
        errorUrl.searchParams.set('invited', 'true');
        router.replace(errorUrl.pathname + errorUrl.search);
      }
    })();
  }, [invitationId]);

  return (
    <div className="animate-fade-in-up space-y-6">
      <div className="text-center">
        <h1 className="text-2xl font-medium tracking-tight text-primary">
          BlockVisor
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">
          Accepting invitation...
        </p>
      </div>

      <div className="space-y-4 rounded-lg border border-border bg-card p-6 text-center">
        <Loader2 className="mx-auto h-10 w-10 animate-spin text-primary" />
        <p className="text-sm text-muted-foreground">
          Please wait while we process your invitation.
        </p>
      </div>
    </div>
  );
}
